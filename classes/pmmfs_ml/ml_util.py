...
def get_amended_day(df,day_df,day):
    ...
    day_df = day_df.merge(df,left_on=["date_" + day,"iso_code"],right_on=["date_updated","iso_code"],how="left",suffixes=["","_" + day])

    # Drop the index of the merged dataframe
    day_df = day_df.reset_index(drop=True)

    # Choose the useful columns from the merged dataframe
    day_df = day_df[["date_updated","iso_code","date_" + day,"total_cases_updated","total_deaths_updated"]]

    # Rename the total_cases and total_deaths case to identify to which future period they belong
    day_df.rename(columns={"total_cases_updated":"total_cases_" + day,
